class SmoothScroll {
  // class to create a smooth scroll on the page using Lenis and GSAP
    ...
    init() {
      ...
      gsap.ticker.add((time) => { // add a ticker to the gsap ticker
    ...
    setupAnchorLinks() {
      ...
      document.querySelectorAll('a[href^="#"]').forEach(anchor => { // for each anchor link
        anchor.addEventListener('click', (e) => { // add an event listener to the anchor link
