window.createHeader = () => {
  ...
  const performSearch = () => { // perform the search
    const query = searchBar.value.trim();
    if (query) { // if the query is not empty
      if (window.location.pathname === '/search.html') { // if the current page is the search page
  ...
  searchBar.addEventListener('keypress', (e) => { // add an event listener to the search bar
  ...
  const showSuggestion = (movie) => { // show the suggestion
    ...
    suggestionContainer.innerHTML = `
      ...
      <div>
        <h3>${movie.Title}</h3>
        <p>${movie.Plot.substring(0, 100)}...</p> // display the plot of the movie
      </div>
    `;
  ...
  const checkCacheAndSuggest = async (query) => { // check the cache and suggest the movies
    try {
      const response = await fetch(`/api/movies/search?query=${encodeURIComponent(query)}`); // fetch the movies
      const data = await response.json();
      if (data.Search && data.Search.length > 0) {
        const movieDetails = await fetch(`/api/movies/details?id=${data.Search[0].imdbID}`); // fetch the movie details
  ...
  searchBar.addEventListener('input', () => { // add an event listener to the search bar
    const existingSuggestion = header.querySelector('.search-container .suggestion-container'); // get the existing suggestion
    if (existingSuggestion) { // if the existing suggestion is not null
    ...
    if (query.length >= 4) { // if the query is longer than 4 characters
      searchTimeout = setTimeout(() => { // set a timeout
